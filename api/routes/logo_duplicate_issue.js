  ...
  try {
    ...
    const layers = layersRes.rows.map(row => {
      const baseLayer = {
        ...
        scaleFactor: row.scale,
        rotation: row.rotation_deg,
        opacity: row.opacity,
        flip: {
          horizontal: false, // Default value, can be extended if needed
          vertical: false    // Default value, can be extended if needed
        }
      };
      ...
      switch (row.type) {
        case 'TEXT':
          return {
            ...
            text: {
              ...
              lineHeight: row.line_height || 1.0,
        ...
        case 'ICON':
          return {
            ...baseLayer,
            icon: {
              src: row.asset_name || `icon_${row.asset_id}`,
        ...
        case 'SHAPE':
          return {
            ...
            shape: {
              ...
              strokeWidth: row.shape_stroke_width || 0
            }
          };
    ...
    const mobileResponse = {
      ...
      templateId: logo.is_template ? logo.id.toString() : null,
      userId: logo.owner_id || 'current_user',
      name: logo.title,
      description: `Logo created on ${new Date(logo.created_at).toISOString()}`,
      canvas: {
...
router.post('/mobile', async (req, res) => {
  ...
  try {
    ...
    // Create logo
    const logoRes = await client.query(`
      INSERT INTO logos (id, owner_id, title, canvas_w, canvas_h, dpi, is_template)
      VALUES ($1, $2, $3, $4, $5, $6, $7)
      RETURNING *
    `, [
      logoId || null,
      userId || null,
      name,
      canvas?.aspectRatio ? 1080 : 1080,
      canvas?.aspectRatio ? Math.round(1080 / canvas.aspectRatio) : 1080,
      300,
      !!templateId
    ]);
    ...
    for (const layerData of layers) {
      ...
      // Map mobile type to database type
          ...
          if (icon) {
            ...
            if (!assetId) {
              const newAssetRes = await client.query(`
                INSERT INTO assets (kind, name, url, width, height, has_alpha)
                VALUES ('vector', $1, '', 100, 100, true)
                RETURNING id
              `, [icon.src]);
            ...
            await client.query(`
              INSERT INTO layer_icon (layer_id, asset_id, tint_hex, tint_alpha)
              VALUES ($1, $2, $3, $4)
            `, [layer.id, assetId, icon.color || '#000000', 1.0]);
          }
          break;

        case 'IMAGE':
          if (image) {
            // Find or create asset for image
            const assetRes = await client.query(`
            ...
            if (!assetId) {
              const newAssetRes = await client.query(`
                INSERT INTO assets (kind, name, url, width, height, has_alpha)
                VALUES ('raster', $1, $2, 100, 100, true)
                RETURNING id
              `, [image.src, image.src]);
              assetId = newAssetRes.rows[0].id;
            }
            ...
            await client.query(`
              INSERT INTO layer_image (layer_id, asset_id, fit)
              VALUES ($1, $2, $3)
            `, [layer.id, assetId, 'contain']);
          }
          break;
        ...
        case 'BACKGROUND':
          if (background) {
            ...
            `, [
              ...
              1.0,
              null // asset_id for background images
            ]);
          }
          break;
      }
    ...
    res.status(201).json({
      ...
      data: {
        ...
        name: logo.title,
        description: description || `Logo created on ${new Date(logo.created_at).toISOString()}`,
        canvas: {
          aspectRatio: logo.canvas_h ? logo.canvas_w / logo.canvas_h : 1.0,
          background: {
            type: 'solid',
  ...
  try {
    ...
    const layers = layersRes.rows.map(row => {
      const baseLayer = {
        ...
        opacity: row.opacity,
        flip: {
          horizontal: row.flip_horizontal || false,
          vertical: row.flip_vertical || false
        }
      };
...
router.post('/mobile', async (req, res) => {
  ...
  try {
    ...
    // Create logo with all new fields
    // Handle userId - if it's not a valid UUID, create a user record or use a default
    let ownerId = userId;
    if (userId && !userId.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)) {
      // If userId is not a UUID, try to find or create a user
      try {
        const userRes = await client.query(`
          SELECT id FROM users WHERE email = $1 LIMIT 1
        `, [userId]);
        ...
        if (userRes.rows.length > 0) {
          ...
          // Create a new user with the userId as email
          const newUserRes = await client.query(`
            INSERT INTO users (email, display_name) VALUES ($1, $2) RETURNING id
          `, [userId, userId]);
    ...
    // Build dynamic query based on whether logoId is provided
    let logoRes;
    if (logoId) {
    ...
    for (const layerData of layers) {
      ...
      // Create base layer - handle layerId similar to logoId
      let layerRes;
      if (layerId && layerId.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)) {
        ...
        layerRes = await client.query(`
          ...
          RETURNING *
        `, [
          layerId, logo.id, dbType, `${type}_layer_${order}`, order,
          position.x, position.y, scaleFactor, rotation, 0.5, 0.5,
          opacity, 'normal', visible, false, flip.horizontal, flip.vertical
        ]);
      } else {
        // No valid UUID, let database generate one
        layerRes = await client.query(`
          INSERT INTO layers (
            logo_id, type, name, z_index, x_norm, y_norm, scale, rotation_deg,
            anchor_x, anchor_y, opacity, blend_mode, is_visible, is_locked,
          ...
          RETURNING *
        `, [
          logo.id, dbType, `${type}_layer_${order}`, order,
          position.x, position.y, scaleFactor, rotation, 0.5, 0.5,
          opacity, 'normal', visible, false, flip.horizontal, flip.vertical
        ]);
      }
