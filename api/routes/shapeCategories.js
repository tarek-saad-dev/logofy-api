...
router.patch('/:id', async(req, res) => {
    ...
    `, [id]);
        ...
        const category = {
            ...
            isActive: cat.is_active,
            meta: cat.meta,
            shapeCount: (countResult.rows[0] && countResult.rows[0].shape_count) || 0,
            createdAt: new Date(cat.created_at).toISOString(),
            updatedAt: new Date(cat.updated_at).toISOString()
        };
...
router.get('/:id/shapes', async(req, res) => {
    try {
        ...
        // Verify category exists and get localized name
    ...
    `, [id]);
        ...
        const cat = categoryCheck.rows[0];
        const localizedCategoryName = lang === 'ar' ?
            (cat.name_ar || cat.name_en || cat.name) :
            (cat.name_en || cat.name);
...
router.post('/:id/shapes', async(req, res) => {
    try {
        ...
        // Verify category exists and get localized name
        const { lang: assignLang = 'en' } = req.query;
        const categoryCheck = await query(`
      SELECT id, name, name_en, name_ar 
      FROM shape_categories 
      WHERE id = $1
    `, [id]);
        ...
        const cat = categoryCheck.rows[0];
        const localizedCategoryName = assignLang === 'ar' ?
            (cat.name_ar || cat.name_en || cat.name) :
            (cat.name_en || cat.name);
