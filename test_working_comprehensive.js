...
// Read the working comprehensive logo JSON
const fs = require('fs');
const workingLogo = JSON.parse(fs.readFileSync('./FINAL_COMPREHENSIVE_LOGO_JSON_WORKING.json', 'utf8'));

async function testWorkingComprehensiveLogo() {
  console.log('🧪 Testing Working Comprehensive Logo with All Attributes\n');
  
  try {
    // Test 1: Create comprehensive logo
    console.log('1️⃣ Creating working comprehensive logo with all attributes...');
    const createResponse = await axios.post(`${BASE_URL}/mobile`, workingLogo);
    ...
    if (createResponse.data.success) {
      ...
      console.log(`   Category ID: ${createResponse.data.data.categoryId}`);
      ...
      if (getResponse.data.success) {
        ...
        console.log('\n🎉 Working comprehensive logo test completed successfully!');
...
// Run the test
testWorkingComprehensiveLogo().then(() => {
  console.log('\n🎉 Working comprehensive logo test finished!');
}).catch(error => {
